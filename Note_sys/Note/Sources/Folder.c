...
Status moveFolder(char* oldPath, char* newPath) {
	// 定义源文件夹和目标文件夹的路径
	
	LPCWSTR source = _T(oldPath);
	LPCWSTR destination = _T(newPath);

	// 调用MoveFile函数，如果成功返回TRUE，否则返回FALSE
	BOOL result = MoveFile(source, destination);
...
int main() {
	...
	createNote("zh", "test", "J:\\Programing\\C\\QG_Training\\Note_sys\\Note\\md\\a");
	return 0;
}
