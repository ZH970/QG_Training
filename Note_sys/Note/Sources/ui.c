...
{ 
	...
	if(strstr(command," "))
	{
		...
		{
			if(strcmp(deleteNoteFromFolder(folder,c[1]),"SUCCESS")==0)// 删除笔记文件名.ooo
			{
				printf("成功！\n");
			}else{
				printf("失败！\n");
		...
		}else if(strcmp("cd",c[0])==0)
		{
			...
			if(strcmp(Folder* moveUpDirectory(Folder* currentFolder),"SUCCESS")==0)//将路径切换为当前目录的父目录. 若是根目录则则不进行切换
			{
				printf("成功！\n");
			}else{
				printf("失败！\n");
